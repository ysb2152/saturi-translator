...
**온디바이스 변환기 대안(권장 순):**
1. **react-native-executorch (Software Mansion)** — ✅ **빌드 통과 확인(2026-09-04, RN0.86)**: react-native-executorch 0.9.3 설치+빌드 `BUILD SUCCESSFUL`, APK에 `libexecutorch.so`·`libreact-native-executorch.so`(arm64+x86_64) + whisper.rn 공존(264MB). onnxruntime과 달리 이 스택에서 빌드됨. 커스텀 모델은 `useExecutorchModule`(Module API)로 임의 .pte 실행 지원. **남은 일**: (i) KoBART encoder/decoder를 `torch.export → executorch(.pte)`로 변환, (ii) JS에서 `useExecutorchModule`로 로드 + encoder→decoder 자기회귀 생성 루프 + KoBART 토크나이저(tokenizer.json) 구현. (빌드 시 whisper.rn CMake 중간산출물 삭제 실패 플레이크는 java 종료+cxx dir 삭제 후 재빌드로 해결.)
