...
**온디바이스 변환기 대안(권장 순):**
1. **react-native-executorch (Software Mansion)** — 최신 RN/Expo·New Arch를 겨냥해 능동 유지되는 온디바이스 ML 런타임. KoBART를 `torch.export → .pte`로 변환 후 JS 토크나이저+생성루프. 이 스택에서 **빌드 성공 확률이 가장 높음** → 다음 세션 1순위 시도(whisper.rn처럼 빈 빌드 검증 먼저).
