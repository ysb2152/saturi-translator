...
> ⚠️ **RN 런타임 비호환 발견(2026-09-03)**: `onnxruntime-react-native` 1.24.3을 설치·prebuild 후 `assembleDebug`가 **BUILD FAILED** — `A problem occurred configuring project ':expo'. > SoftwareComponent with name 'release' not found`(gradle 9.3.1). 웹에서 본 "onnxruntime-react-native가 Expo에서 안 됨" 이슈가 이 스택(SDK57/RN0.86/gradle9)에서 실제로 재현됨. 되돌려(제거) whisper.rn-only green 빌드를 유지. **모델 export/양자화(위 ✅)는 문제없고, 막힌 건 RN 네이티브 런타임 통합**이다. 대응(§6-1 폴백): (a) **하이브리드** — STT는 온디바이스(검증됨), 변환기(KoBART)는 서버(음성은 기기를 안 벗어나고 인식된 텍스트만 전송); (b) 호환 조합 탐색 — onnxruntime-react-native 구버전 핀·gradle/AGP 다운·patch-package; (c) 대체 런타임 — `react-native-executorch`나 TFLite. 추가 조사(2026-09-03): 실패 지점은 `components.release`를 참조하는 **expo/RN gradle 플러그인의 모듈 publishing 컨벤션**으로, `:onnxruntime-react-native`와 `:expo` 양쪽이 걸린다. 라이브러리 android 블록에 `publishing { singleVariant("release") }`를 넣는 표준 패치로는 **해결 안 됨**(whisper.rn은 통과하는데 onnxruntime은 안 됨 — 모듈 구조 차이). 단순 patch 범위를 넘는 gradle-9/AGP 상호작용 문제.

**온디바이스 변환기 대안(권장 순):**
1. **react-native-executorch (Software Mansion)** — ✅ **빌드 통과 확인(2026-09-04, RN0.86)**: react-native-executorch 0.9.3 설치+빌드 `BUILD SUCCESSFUL`, APK에 `libexecutorch.so`·`libreact-native-executorch.so`(arm64+x86_64) + whisper.rn 공존(264MB). onnxruntime과 달리 이 스택에서 빌드됨. 커스텀 모델은 `useExecutorchModule`(Module API)로 임의 .pte 실행 지원. ✅ **(i) 모델 .pte 변환도 성공(2026-09-04)**: KoBART encoder + decoder(단일스텝, KV캐시 없음)를 `torch.export → to_edge → to_executorch`로 변환. **encoder.pte 265.5MB + decoder.pte 322.5MB**(fp32, portable). 재현: [`backend/export_kobart_pte.py`](../backend/export_kobart_pte.py), 별도 venv(`C:/et`, executorch 1.4.1+torch2.14) — 학습 venv 미오염. 함정: (a) torch 긴경로 → venv를 짧은 경로(`C:/et`)에; (b) XNNPACK 백엔드 직렬화는 `flatc` 바이너리 필요(미설치) → **portable로 우회**(flatc 없이 성공). ✅ **(ii) JS 통합도 기기에서 로드·실행 확인(2026-09-04)**: [`mobile/src/ondeviceConverter.js`](../mobile/src/ondeviceConverter.js) — `TokenizerModule`(tokenizer.json) + `ExecutorchModule`×2(encoder/decoder .pte) + greedy 생성 루프. 에뮬레이터에서 **tokenizer+encoder+decoder 로드 성공, 생성 루프 실행됨**(RnExecutorch 네이티브 정상). 함정: `initExecutorch({resourceFetcher: ExpoResourceFetcher})` 필수 + 어댑터 패키지 `react-native-executorch-expo-resource-fetcher` 0.9.1의 **package.json `exports`에 default/require 조건이 없어 metro resolve 실패**(patch-package로 exports 제거 필요; 테스트 땐 src로 vendoring해 우회). int64 입력은 `BigInt64Array`(ScalarType.LONG).

✅ **(1) 빈 출력 해결 → 온디바이스 변환 성공(2026-09-04)**: 에뮬레이터에서 **"밥 문나 아직 안 무따" → "밥 문나 아직 안 먹었다"**(사투리→표준어), **8.5초**. 디버깅 경로: ① Python executorch 런타임으로 .pte greedy 재현이 PyTorch와 완전 일치 → 모델·로직은 정확, 버그는 JS 레이어로 국한([`backend/debug_pte_gen.py`](../backend/debug_pte_gen.py)). ② JS 진단 로그로 분기점 특정 — 토크나이저 ids는 정확([17108,…], Python과 동일), **실제 버그는 decoder 출력 `logits.dataPtr`가 Float32Array가 아니라 ArrayBuffer**여서 `buf.length`가 undefined→argmax가 best=0 고정→빈 출력. **수정**: 출력을 `new Float32Array(dataPtr)`로 감싸고, 겸사겸사 int32 입력으로 재-export(Int32Array, BigInt64Array 회피). (참고: 버그 땐 best가 EOS를 못 만나 64스텝 전부 돌아 273초였고, 정상화 후 7스텝에 조기 종료라 8.5초로 빨라짐.) 리소스페처는 어댑터 패키지의 metro resolve 실패로 `src/etfetcher`에 vendoring. (2) **매우 느림 — 문장당 ~273초**(portable 커널+KV캐시 없음+에뮬 x86_64 CPU). 실용화엔 (iii) **최적화 필수**: int8 양자화 + XNNPACK 델리게이트(flatc 필요) + **decoder KV캐시 export**(단일스텝 O(n²) 제거) + 실기기(arm). (빌드 시 whisper.rn CMake 삭제 플레이크는 java 종료+cxx dir 삭제 후 재빌드로 해결.)
2. **gradle 8.x 핀** — 비호환이 gradle 9 특유일 수 있어, wrapper를 8.x로 낮춰 onnxruntime 빌드 시도(단, Expo SDK57/AGP가 gradle 9 요구할 위험).
3. **onnxruntime-react-native 다른 버전 핀** / patch-package 심화.
4. (최후) 하이브리드 — STT 온디바이스 + 변환기 서버(음성은 기기 밖으로 안 나감).
