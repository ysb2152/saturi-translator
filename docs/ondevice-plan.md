...
⚠️ **남은 문제 2가지**: (1) **출력이 빈 문자열** — Python에서 executorch 런타임으로 같은 .pte + 같은 greedy 루프를 돌린 결과는 **PyTorch와 완전 일치**("밥 문나 아직 안 먹었다", enc_hidden diff 1.5e-6). 재현: [`backend/debug_pte_gen.py`](../backend/debug_pte_gen.py). 즉 **.pte·생성로직은 정확하고, 빈 출력은 JS 통합 레이어 버그로 국한**됨 — 유력 원인: RNE `TokenizerModule.encode` 출력이 HF와 다름(KoBART는 인코더 입력에 special token을 안 붙임 — [17108,…] 형태), 또는 int64 입력(`BigInt64Array`)을 executorch 네이티브가 다르게 읽음. 다음: JS 하네스에 진단 로그(encoded ids·첫 로짓 top-k) 넣어 1회 실행으로 분기점 특정, 또는 int32 입력으로 재-export(Int32Array 사용). (2) **매우 느림 — 문장당 ~273초**(portable 커널+KV캐시 없음+에뮬 x86_64 CPU). 실용화엔 (iii) **최적화 필수**: int8 양자화 + XNNPACK 델리게이트(flatc 필요) + **decoder KV캐시 export**(단일스텝 O(n²) 제거) + 실기기(arm). (빌드 시 whisper.rn CMake 삭제 플레이크는 java 종료+cxx dir 삭제 후 재빌드로 해결.)
