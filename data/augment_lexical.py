# 어미 말고 '단어 자체'가 바뀌는 사투리를 보강한다.
#
# 비공개 테스트에서 맡기다→매끼다 처럼 동사·부사·대명사가 통째로 바뀌는 건
# 어미 증강(augment_endings.py)으로는 안 잡힌다는 피드백을 받아서 만들었다.
# 진짜 표준어 문장의 어절 중 사전에 있는 표준 표면형을 사투리 표면형으로 바꿔 (합성 사투리, 표준어) 쌍을 만든다.
# 활용이 틀어지는 걸 막으려고 어절 정확일치만 쓰고, 중의적인 항목은 뺀다.
#
# 사용:
