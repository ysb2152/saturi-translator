# 종결어미 합성 증강 — 학습 데이터에 거의 없는 지역 대표 어미를 인공 생성해 보강.
#
# 근거: training/ending_coverage.md — 랑께·능교·드래·임다 등 지역 상징 어미가 학습쌍에 사실상 0건.
# 방법: 실제 '표준어' 문장(데이터의 standard 측)에 신뢰도 높은 어미 치환을 역으로 적용해
#        (합성 사투리, 표준어) 쌍을 만든다. 표준측은 진짜 문장이라 자연스럽고, 사투리측은 어미만 바뀐다.
#
# 원칙:
#  - 문장 끝(마지막 어절)에서만, 표면형이 명확히 매칭될 때만 치환(오생성 억제).
#  - 규칙당 상한(--cap)으로 균형. 한 문장엔 첫 매칭 규칙 하나만 적용.
#  - 형태론적으로 안전한 접미사 치환 + 소수의 축약형 사전만 사용(무리한 활용 변형 배제).
#
# 사용:
