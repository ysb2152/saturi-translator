"""AI Hub 한국어 방언 발화 라벨(JSON)을 파싱하고 전사 텍스트를 정제한다.

AI Hub 방언 데이터는 연도·지역마다 라벨 JSON 필드명이 조금씩 달라서,
가장 흔한 구조를 기본으로 두고 없는 필드는 후보를 순서대로 훑어 대체(fallback)하게 짰다.
새 파일을 받아도 보통 아래 FIELDS만 맞추면 돌아간다.
