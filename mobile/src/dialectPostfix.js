// 변환기(KoBART)가 놓친 사투리 표현을 규칙으로 마지막에 한 번 더 보정하는 안전망.
// 모델을 대체하려는 게 아니라, 출력에 남은 명백하고 문맥 안 타는 표현만 살짝 고친다.
// 니→너/네, 아이가→않니, 내→나 처럼 한 어절이 여러 표준형이 될 수 있는 것,
// 따른/그래/그러고/있다 처럼 표준어에도 있는 형태는 오탐이 나서 일부러 뺐다 — 그건 모델이 판단할 몫.
// 오탐을 줄이려고 치환은 어절(공백) 단위 정확 일치만 한다.
// 넣은 항목은 data/analysis.md 고빈도 치환 Top30 중 무맥락·비중의적인 것들이고,
// 경상 존대어미 '아임다/임더'는 data/ 전수검색에서 학습쌍에 0건인 걸 확인하고 추가했다.
