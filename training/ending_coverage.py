# 사투리 종결어미 커버리지 진단 — 변환(MT) 학습 데이터에서 어떤 어미가 실제로 부족한지 정량화.
#
# 두 가지를 본다:
#  (1) 큐레이션한 지역 종결어미 목록 각각이 '방언 측'에 얼마나 등장하고, 그중 실제로 변형(방언≠표준)된 쌍이 몇 개인지.
#      → 등장·변형이 적을수록 모델이 그 어미를 표준으로 바꾸는 법을 못 배운다(커버리지 공백).
#  (2) 실제 변형된 쌍에서 '마지막 어절'의 치환 Top — 무엇이 잘 커버되는지 대조군.
#
