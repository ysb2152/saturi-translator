...
- **① STT** — `faster-whisper`. 표준 Whisper를 사투리 음성으로 파인튜닝해 인식률 개선(예정).
...
→ 규칙 사전으로는 다 담기 어려운 이 다양한 변형을 **모델이 데이터로 학습**한다. 실제로 KoBART 변환 모델은 '그대로 두기(copy)' 기준선 대비 **CER 4.62% → 1.01%, 문장 정확일치 29.6% → 82.2%**로 개선됐다(val 500). 전체 분석은 [data/analysis.md](data/analysis.md), 파인튜닝 설계·근거는 [why_finetune.md](why_finetune.md) 참고.
...
- [x] KoBART 변환 모델 파인튜닝 — **CER 4.6%→1.0%, 정확일치 30%→82%** (copy 기준선 대비) + 백엔드 연동
- [ ] Whisper STT 파인튜닝(표준 대비 CER/WER 개선 측정)
- [ ] 플레이스토어 배포
