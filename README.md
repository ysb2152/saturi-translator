...
- **① STT** — Whisper. 표준 Whisper를 사투리 음성으로 파인튜닝해 인식률 개선(**CER 18.6%→10.2%**). 서빙은 `faster-whisper`.
...
그리고 **STT(Whisper)** 도 표준 대비 사투리 인식 CER을 **18.6% → 10.2%(45% 개선)** 로 낮췄다. 즉 음성 인식·표준어 변환 **두 단계 모두** "표준 모델이 못하는 것을 데이터로 개선"을 정량 증명했다.
...
- [x] Whisper STT 파인튜닝 — 표준 대비 **CER 18.6%→10.2%**(45% 개선), 로컬 GPU 학습
- [ ] 플레이스토어 배포
